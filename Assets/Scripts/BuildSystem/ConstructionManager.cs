...
public class ConstructionManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            // if placing a floor
            if (itemToBeConstructed.name == "FloorModel")
            {
            ...
            if (Physics.Raycast(ray, out hit))
            {
                ...
                if (selectionTransform.gameObject.CompareTag("ghost") && itemToBeConstructed.name == "FloorModel")
                {
        ...
        {
            if (isValidPlacement && selectedGhost == false && itemToBeConstructed.name == "FloorModel") // We don't want the freestyle to be triggered when we select a ghost.
            {
                PlaceItemFreeStyle();
        ...
        // X key to Cancel                      //TODO - don't destroy the ui item until you actually placed it.
    ...
    private void PlaceItemInGhostPosition(GameObject copyOfGhost)
    {
        ...
        var randomOffect = UnityEngine.Random.Range(0.01f, 0.03f);

        itemToBeConstructed.transform.position = new Vector3(ghostPosition.x, ghostPosition.y, ghostPosition.z + randomOffect);
